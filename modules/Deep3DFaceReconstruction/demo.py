...
def demo():
	...
	image_path = '/mnt/mfs/chenjianyi/records/Deepfake/face2face/face_frames/003'
	save_path = '/mnt/mfs/chenjianyi/records/Deepfake/face2face/face_3dmm_weakly_sup/003'
	img_list = glob.glob(image_path + '/' + '*.jpg')

	img_list = ['WechatIMG19.jpeg']
	save_path = 'out'
	...
	with tf.Graph().as_default() as graph,tf.device('/cpu:0'):
		...
		graph_def = load_graph('/mnt/mfs/chenjianyi/project/Deepfake/Deep3DFaceReconstruction/network/FaceReconModel.pb')
		...
		with tf.Session() as sess:
			...
			for file in img_list:
				...
				save_obj(os.path.join(save_path,file.split('/')[-1].replace('.jpeg','_0.obj')),shape,tri,np.clip(color,0,255)/255) # 3D reconstruction face (in canonical view)
