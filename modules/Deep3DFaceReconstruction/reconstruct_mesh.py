...
def Projection_layer(face_shape,rotation,translation,focal=1015.0,center=112.0): # we choose the focal length and camera position empirically
	...
	aug_projection[:, :, 0:2] = aug_projection[:, :, 0:2] / np.reshape(aug_projection[:,:,2],[1,np.shape(aug_projection)[1],1])
	face_projection = aug_projection[:,:,0:3]
	face_projection = np.matmul(face_projection,reverse_z)
	z_buffer = np.reshape(aug_projection[:,:,2],[1,-1,1])

	return face_projection,z_buffer

def Render_prolection(vertices, params, shift_to_origin=True):
	"""
	vertices: [N, 3]
	params: dict
	"""
	...
	face_projection,z_buffer = Projection_layer(vertices, rotation, translation)
	face_projection = np.squeeze(face_projection, (0))  # [N, 3]
	vertices = np.stack([face_projection[:,0],224 - face_projection[:,1], face_projection[:,2]], axis = 1)

	if shift_to_origin:	
		w0, h0, s, t0, t1 = transform_params[0], transform_params[1], transform_params[2], transform_params[3], transform_params[4]
		vertices = vertices / s
		vertices[:, 0] += (t0 - w0 / 2)
		vertices[:, 1] += (t1 - h0 / 2)
	
		lm, kpt_idx_3d = params['lm'], params['kpt_idx_3d']
		lm3d = vertices[kpt_idx_3d, :]
		left_eye = lm[0]
		left_eye_3d = (lm3d[36] + lm3d[39]) / 2
		vertices[:, 0] += (left_eye[0] - left_eye_3d[0])
		vertices[:, 1] += (left_eye[1] - left_eye_3d[1])
	return vertices
...
def Reconstruction(coeff,facemodel):
	...
	face_projection,z_buffer, *_ = Projection_layer(face_shape,rotation,translation)
